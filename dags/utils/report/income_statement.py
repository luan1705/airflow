...
def update_one_income_statement_symbol(symbol: str, page: str):
    """
    Chỉ thêm quý/năm mới cho một symbol.
    Không đụng METRIC.
    """
    try:
        data, _ = fetch_income_statement(
            symbol,
            page,
        )
...
def _run_income_statement_update(
    symbols,
    page,
):
    print(
    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        futures = {
            executor.submit(
                update_one_income_statement_symbol,
                symbol,
                page,
            ): symbol
...
def income_statement_update():
    ...
    return _run_income_statement_update(
        symbols,
        page,
    )
