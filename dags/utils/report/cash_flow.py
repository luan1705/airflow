...
def fetch_cash_flow(symbol: str, page: str, max_retries=3):
    headers = {
...
def save_one_symbol(symbol: str, page: str):
    try:
        data, metric = fetch_cash_flow(symbol, page)
        if data.empty:
...
def cash_flow_missing():
    """Chỉ chạy các symbol chưa có bảng trong schema cash_flow."""
    page = "CASH_FLOW"
