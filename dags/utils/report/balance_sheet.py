...
def insert_new_periods(conn, schema: str, table: str, df: pd.DataFrame):
    ...
    # Nếu mã chưa có bảng thì tạo như logic full
    if not table_exists:
...
def update_one_symbol(symbol: str, page: str):
    """
    Chỉ thêm quý/năm mới cho một symbol.
    Không đụng METRIC.
    """
    try:
        data, _ = fetch_balance_sheet(
            symbol,
            page,
        )
...
def _run_update_symbols(symbols, page):
    print(
