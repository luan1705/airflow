...
def fetch_note(symbol: str, page: str, max_retries=3):
    headers = {
...
def save_one_symbol(symbol: str, page: str):
    try:
        data, metric = fetch_note(symbol, page)
        if data.empty:
...
def note_missing():
    """Chỉ chạy các symbol chưa có bảng trong schema note."""
    page = "NOTE"
...
def update_one_note_symbol(symbol: str, page: str):
    """
...
def _run_note_update(symbols, page):
    print(
