...
def ssi_tradingview_1(symbol, token):

    def fetch_minute_ohlc(token: str, symbol: str, from_date: str, to_date: str, page_index: int = 1, page_size: int = 1000):
        headers = {"Authorization": f"Bearer {token}", "Accept": "application/json", "Content-Type": "application/json"}
        params = {"symbol": symbol, "from": from_date, "to": to_date, "timeFrame": "1m", "pageIndex": page_index, "pageSize": page_size}
    ...
    def to_df(rows):
        ...
        if "tradingDate" in df.columns:
            df["tradingDate"] = pd.to_datetime(df["tradingDate"], format="%Y/%m/%d %H:%M:%S", errors="coerce")
    ...
    rows = fetch_minute_ohlc(token, symbol, start, end, page_index=1, page_size=1000)
    df = to_df(rows)
    return df
...
if __name__ == "__main__":
    ...
    df = ssi_tradingview_1(symbol, token)
    print(df)