...
def foreign_UPCOM():
    try:    
        ...
        df = pd.read_sql('SELECT * FROM "market_history"."foreign_UPCOM_1"', con=enginedb)
        ...
        foreign_UPCOM=pd.DataFrame([['buy_Vol',buy_Vol],
                            ['sell_Vol',sell_Vol],
                            ['net_Vol',net_Vol],
                            ['buy_Val',buy_Val],
                            ['sell_Val',sell_Val],
        ...
        foreign_UPCOM.to_sql(name='foreign_UPCOM',
                             schema='market_data',
                             con=enginedb,
                             if_exists='replace',
                             index=False
                            )
        logging.info('Đã lưu foreign_UPCOM')
        ...
        logging.exception('Lỗi lưu foreign_UPCOM')
