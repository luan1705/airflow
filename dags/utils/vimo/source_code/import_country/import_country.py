...
def find_import_country_chart(file_path: str) -> list[dict]:
    """
    Tự tìm chart có:
        - Series nhập khẩu hàng hóa
        ...
        for chart_file in chart_files:
            try:
                ...
                if has_export_series and has_required_countries:
                    log.info(
                    "Tìm thấy chart nhập khẩu quốc gia: %s",
                    chart_file,
                )
    ...
    raise ValueError(
        f"Không tìm thấy biểu đồ nhập khẩu theo quốc gia "
...
def get_import_series(series_list):
    for series in series_list:
        series_name = normalize_text(series["name"])

        if "nhap khau" in series_name:
            return series

    raise ValueError(
        "Không tìm thấy series 'Nhập khẩu hàng hóa'"
    )
...
def find_import_country_from_text(file_path: str):
    ...
    for key, aliases in countries.items():
        ...
        for country in aliases:
            match = re.search(
                ...
                rf".{{0,150}}?"
                rf"(?:uoc\s+)?"
                rf"(?:dat|voi|kim ngach|nhap khau)?.{{0,40}}?"
                rf"([\d.,]+)\s+ty\s+usd",
                full_text,
...
def parse_import_country(file_path: str) -> pd.DataFrame:
    report_time = parse_time_from_filename(
        file_path
    )

    try:
        series_list = find_import_country_chart(
            file_path
        )

        import_series = get_import_series(
            series_list
        )

        import_values = series_to_dict(
            import_series
        )

        usa_ytd = import_values.get("hoa ky")
        china_ytd = import_values.get("trung quoc")
        korea_ytd = import_values.get("han quoc")
        asean_ytd = import_values.get("asean")
        eu_ytd = import_values.get("eu")
        japan_ytd = import_values.get("nhat ban")

        # Chart có nhưng thiếu bất kỳ nước nào
        # -> fallback sang text
        if (
            usa_ytd is None
            ...
            raise ValueError(
                "Chart import_country thiếu dữ liệu"
            )

        print(
            "✅ Lấy dữ liệu import_country từ chart"
        )
        ...
        import_values = find_import_country_from_text(
            file_path
        )

        usa_ytd = import_values.get("usa")
        china_ytd = import_values.get("china")
        korea_ytd = import_values.get("korea")
        asean_ytd = import_values.get("asean")
        eu_ytd = import_values.get("eu")
        japan_ytd = import_values.get("japan")
...
def upsert_import_country(df: pd.DataFrame):
    records = (
...
def save_import_country(file_path: str):
    df = parse_import_country(file_path)
    print(df.to_string(index=False))
    upsert_import_country(df)
