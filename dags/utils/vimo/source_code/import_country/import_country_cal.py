...
def import_country_cal(**context):
    df = pd.read_sql(
        text(f"""
            SELECT *
            FROM "{SCHEMA}"."{TABLE}"
    ...
    for col in COLS:
        ...
        if ytd_col not in df.columns:
            raise ValueError(
                f"Thiếu cột {ytd_col} trong {SCHEMA}.{TABLE}"
            )
        ...
        # Kiểm tra dòng trước có đúng tháng liền kề hay không
    ...
    print(
        f"✅ Tính xong giá trị tháng nhập khẩu "
        f"cho {len(df)} rows"
    )
