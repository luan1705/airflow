...
def bank_interest_history_1y():
    ...
    if not json_data["data"]:
        log.info("API bank_interest_1y không có dữ liệu mới.")
        return 0
    ...
    if missing_columns:
        raise ValueError(
            f"API bank_interest_1y thiếu columns: {missing_columns}"
        )
    ...
    with engine.begin() as con:
        ...
        con.execute(text("""
            CREATE TABLE IF NOT EXISTS macro.bank_interest_1y (
    ...
    if not rows:
        log.info("Không có dữ liệu bank_interest_1y để ghi DB.")
        return 0

    sql = """
        INSERT INTO macro.bank_interest_1y
        ("time", "sob", "large", "small")
        VALUES %s
