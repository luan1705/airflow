...
def bank_interest_history_3m():
    ...
    if not json_data["data"]:
        log.info("API bank_interest_3m không có dữ liệu mới.")
        return 0
    ...
    if missing_columns:
        raise ValueError(
            f"API bank_interest_3m thiếu columns: {missing_columns}"
        )
    ...
    with engine.begin() as con:
        ...
        con.execute(text("""
            CREATE TABLE IF NOT EXISTS macro.bank_interest_3m (
    ...
    if not rows:
        log.info("Không có dữ liệu bank_interest_3m để ghi DB.")
        return 0

    sql = """
        INSERT INTO macro.bank_interest_3m
        ("time", "sob", "large", "small")
        VALUES %s
