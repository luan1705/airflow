...
def save_price_diff_to_symbol_list(addition: list[str]):
    """
    Ghi list price_diff vào block:
        # <price_diff START>
        price_diff = [...]
        # <price_diff END>
    """

    addition_code = "# <price_diff START>\n\n"
    addition_code += format_symbol_list("price_diff", addition, items_per_line=8)
    addition_code += "\n\n# <price_diff END>"

    content = SYMBOL_LIST_FILE.read_text(encoding="utf-8")

    pattern = r"(?ms)# <price_diff START>.*?# <price_diff END>"

    if not re.search(pattern, content):
        raise RuntimeError(
            f"Không tìm thấy block # <price_diff START> ... # <price_diff END> trong {SYMBOL_LIST_FILE}"
        )
...
def check_all_market_symbols():
    ...
    price_diff = check_all_symbols(all_symbols, max_workers=5)

    log.info(f"✅ Tổng số mã đã check: {len(all_symbols)}")
    log.info(f"❌ Tổng số mã bị lệch: {len(price_diff)}")
    log.warning(f"Symbols bị lệch: {price_diff}")

    save_price_diff_to_symbol_list(price_diff)
    log.info(f"✅ Đã ghi price_diff vào {SYMBOL_LIST_FILE}")

    return None


if __name__ == "__main__":
    check_all_market_symbols()