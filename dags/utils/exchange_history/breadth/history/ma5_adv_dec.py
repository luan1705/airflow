...
def get_value(index_name: str):
    ...
    union = " UNION ALL ".join([
        f'SELECT time, "value" FROM ohlcv."{s}_1D"'
        for s in symbols
    ])
...
def calc_breadth_ma5(index_name: str):
    ...
    return result.dropna(subset=['advancersPctMa5']).reset_index()


def calc_trading_value_weight():
    vnindex = get_value('VNINDEX')
    vn30    = get_value('VN30')
    vnmid   = get_value('VNMID')
    vnsmall = get_value('VNSMALL')
    vn100   = get_value('VN100')

    if any(x is None for x in [vnindex, vn30, vnmid, vnsmall, vn100]):
        print("⚠️ Thiếu dữ liệu trading value.")
        return None
    ...
    with engine.begin() as conn:
        conn.execute(text(f"""
            CREATE TABLE IF NOT EXISTS exchange_history."{table}" (
                ...
                "vnindexValuePct"    DOUBLE PRECISION,
                "tradingValue"       DOUBLE PRECISION
            )
        """))
        conn.execute(text(f"""
            ALTER TABLE exchange_history."{table}"
            ADD COLUMN IF NOT EXISTS "vnindexValuePct" DOUBLE PRECISION
        """))
        conn.execute(text(f"""
            ALTER TABLE exchange_history."{table}"
            ADD COLUMN IF NOT EXISTS "tradingValue" DOUBLE PRECISION
        """))
...
def ma5_adv_dec():
    weights = calc_trading_value_weight()
    trading_values = {
        idx: get_value(idx)
