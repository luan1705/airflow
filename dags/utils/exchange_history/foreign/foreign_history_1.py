...
def foreign_history(symbol, start=None, end=None):
  if start is None:
  ...
  ketthuc = to_timestamp_utc(start,hour=0,   minute=0, second=0)+ 24*60*60
  ...
  data['time'] = pd.to_datetime(data['time'], unit='s', utc=True).dt.tz_convert('Asia/Ho_Chi_Minh')
