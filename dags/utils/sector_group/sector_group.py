...
def sector_group():
    ...
    try:
        ...
        # =========================
        df_trend = df0.dropna(subset=["open", "refPrice", "close", "sectorGroup"]).copy()
        ...
        trend_count = (
            df_trend.groupby("sectorGroup", as_index=False)[["advancers","decliners","noChanges"]]
            .sum()
        )
        ...
        # =========================
        df = df0.dropna(subset=["sectorGroup"]).copy()
        ...
        df["totalMarketCap_sectorGroup"] = df.groupby("sectorGroup")["marketCap"].transform("sum").fillna(0)
        df["marketCapweigh"] = np.where(df["totalMarketCap_sectorGroup"] > 0, mcap / df["totalMarketCap_sectorGroup"], 0.0)
        ...
        df_clean = (
            df.groupby("sectorGroup", as_index=False)
              .agg(
        ...
        # =========================
        df_final = df_clean.merge(trend_count, on="sectorGroup", how="left").fillna(0)
        ...
        update_set = ', '.join(f'"{c}" = EXCLUDED."{c}"' for c in cols if c != 'sectorGroup')
        ...
        with enginedb.begin() as conn:
            with conn.connection.cursor() as cur:
                execute_values(
                    cur,
                    f"""
                        INSERT INTO details.sector_group ({col_list})
                        VALUES %s
                        ON CONFLICT ("sectorGroup") DO UPDATE SET {update_set}
                    """,
                    rows_data,
                    page_size=1000
                )

        logging.info("Đã lưu sector_group với trend count và weight")

    except Exception:
        logging.exception("Lỗi lưu sector_group")
        raise
    finally:
