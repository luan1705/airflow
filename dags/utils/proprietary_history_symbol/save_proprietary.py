...
TARGET_SCHEMA = "asset_propietary_history"  # đúng theo bạn yêu cầu
...
def get_proprietary_history(symbol):
    try:
        ...
        rows = data[["time", "netVol", "netVal"]].to_dict("records")
        ...
        with enginedb.begin() as conn:
            ...
            conn.execute(
                text(f'''
                    INSERT INTO "{TARGET_SCHEMA}"."{table_name}" ("time","netVol","netVal")
                    VALUES (:time, :netVol, :netVal)
        ...
        msg = f"✅ Upsert {symbol} -> {TARGET_SCHEMA}.{table_name} ({len(rows)} dòng) OK"
        log.info(msg)
        return [msg]
