# ============================================================
# FETCH EPS FROM VIETCAP API (Financial Statement - Latest Year Only)
# ============================================================
...
# ================== FETCH EPS (LATEST YEAR ONLY) ================== #
def get_latest_eps(session: requests.Session, symbol: str):
    """
    Lấy EPS của năm mới nhất từ Income Statement (ISA23 field)
    """
    url = f"{BASE_URL}/company/{symbol}/financial-statement"
    params = {"section": "INCOME_STATEMENT"}
    ...
    try:
        ...
        # ✅ Tìm năm mới nhất (yearReport lớn nhất)
        latest = max(years, key=lambda x: x.get("yearReport", 0))
        eps = latest.get("isa23")
...
if __name__ == "__main__":
    ...
    with engine.begin() as conn:
        for i, sym in enumerate(SYMBOLS, 1):
            row = get_latest_eps(session, sym)

            if row:
                conn.execute(UPSERT_SQL, row)
                ok += 1
                print(f"✅ {i}/{len(SYMBOLS)} {sym} EPS={row['eps']} (Year {row['year']})")
            else:
                fail += 1
                print(f"❌ {i}/{len(SYMBOLS)} {sym} NO EPS")
