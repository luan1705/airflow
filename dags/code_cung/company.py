...
# ===================== SYMBOL LIST =====================
# ví dụ, bạn thay bằng HOSE + HNX + UPCOM của bạn
HOSE = [
...
# ===================== VIETCAP IQ COMPANY PROFILE =====================
def get_company_profile_vietcap(symbol: str):
    """
    return:
        {
            symbol,
            name     : tên công ty tiếng Việt
            industry : tên ngành tiếng Anh
        }
    """
...
# ===================== MULTIPLE SYMBOLS =====================
def get_multiple_profiles(symbols: list):
    results = []

    for sym in symbols:
        info = get_company_profile_vietcap(sym.strip().upper())
        if info:
            results.append(info)
...
def save_to_postgres(df: pd.DataFrame):
    ...
    with engine.begin() as conn:
        sql = text("""
            INSERT INTO info.asset (symbol, name, industry, type)
            VALUES (:symbol, :name, :industry, 'Stock')
            ON CONFLICT (symbol)
            DO UPDATE SET
                name = EXCLUDED.name,
...
if __name__ == "__main__":
    ...
    data = get_multiple_profiles(symbols)
