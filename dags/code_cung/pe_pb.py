...
symbols = HOSE + CQ + HNX + HNXBOND + UPCOM + DERIVATIVES + ETFHOSE
def get_financial_ratios(symbol: str):
    ...
    try:
        ...
        # lấy bản ghi mới nhất (dùng nội bộ, KHÔNG lưu year/quarter)
        latest = max(
            data,
            key=lambda x: (int(x.get("year", 0)), int(x.get("quarter", 0)))
        )
    ...
    "sharesOutstanding": latest.get("numberOfSharesMktCap")
}
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=20) as executor:
        futures = {executor.submit(get_financial_ratios, sym): sym for sym in symbols}
...
"""
    ...
    with engine.begin() as conn:
        conn.execute(text(UPSERT_SQL), results)

    print(f"✅ Upsert thành công {len(results)} symbol vào info.asset")